    ...
    while (keepFetching && results.length < limit) {
      ...
      if (
        ...
        pins.forEach(pin => {
          ...
          if (pin.images && pin.images['736x']) entry.url = pin.images['736x'].url;
          else if (pin.images && pin.images['orig']) entry.url = pin.images['orig'].url;
          if (entry.url) results.push(entry);
        });
...
module.exports = { searchPinterestAPI };
